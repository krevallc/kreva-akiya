...
### tamano（玉野市空き家情報 / city.tamano.lg.jp）★玉野市（純増・PDF台帳）
- 玉野市公式。物件リストは1つのPDF「空き家情報（令和◯年◯月◯日）」に集約、**1物件1ページ**の定型。
- 親ページ `/soshiki/20/2387.html` から最新PDFリンクを動的取得（添付番号は更新で変わる）→ `pdfplumber` でテキスト抽出してパース。
- 取得：登録番号(source_id)・所在地→住所・売買価格・敷地/建物面積・構造・建築年(和暦)・用途地域（市街化調整区域はkuiki_kubunへ）・利用状況/補修/設備/学区(説明文)。
- 所在地は町名のみ→国土地理院ジオコーディング。写真はPDF内のみ（地図サムネfallback）。**要 pdfplumber**（requirements.txt）。
- 検証（2026-07-08・実PDF）：31頁から30物件を抽出。価格23件/応談7件、市街化調整区域5件。
