...
### yakage（矢掛町空き家情報バンク / YAKAGE LIFE）★矢掛町（住まいる岡山**未参加＝純増**）
- 移住支援サイト YAKAGE LIFE。robots無し（許可扱い）。※**HTTP専用**（HTTPSは証明書エラー）。
- 一覧 `/ijyu/vacant-house/house.html?page=N`（ページ送り）→ 詳細 `/ijyu/vacant-house/<番号>.html`（th/td）。
- 取得：所在地(町名)→住所ジオコーディング・売却/賃貸価格・構造・設備(説明文)・番号(source_id)。
- 写真は在るが**HTTP配信**のため HTTPS の kreva.co.jp では混在コンテンツでブロック → 取り込まず地図サムネfallback。
- 検証（2026-07-08・Chrome実データ）：No.359 東川面 売却600万・木造瓦葺2階建、賃貸のみ→価格応談、商談中→成約済 の分岐OK。

### misaki（美咲町空き家バンク / misaki-akiyabank.com）★美咲町（独立サイト＝純増）
- 美咲町の独立した空き家バンク（WordPress・住まいる岡山/アットホームとは別の一次ソース）。robots は /wp-admin/ のみDisallow。
- takahashi と同構造：`wp-sitemap-posts-bank-1.xml` で物件URL `/bank/<番号>/`（約35件）→ 詳細 th/td をパース。
- 取得：登録番号(source_id)・所在地→住所・売買価格・敷地/延床面積・間取り・構造・設備/周辺/特記(説明文)。
- 座標は Google マップ埋め込み `!2d<経度>!3d<緯度>`（エリア単位）。写真は wp-content/uploads（**HTTPS＝表示可**）。
- 検証（2026-07-08・Chrome実データ）：MC002 打穴中 売買1180万・7DK・敷地1054.66㎡・座標(35.008,133.929)・写真取得OK。

### shinjo（新庄村空き家情報バンク / vill.shinjo.okayama.jp）★新庄村（純増・小規模）
- 新庄村（真庭郡）の自前バンク（定住促進サイト・CMS）。robots無し。※**HTTP専用**（写真も地図フォールバック）。
- 一覧 `index.php?id=100` の「情報提供中の物件」→ 物件番号NN → 詳細 `index.php?id=<詳細id>`（2列td/tdテーブル）。
- 取得：物件番号(source_id)・売却価格（多くは金額要相談）・土地/延床面積・間取り・構造・設備/周辺/備考。
- **所在地の掲載が無い**ため村中心をジオコーディング（岡山県真庭郡新庄村）。現在1件だが一覧から自動追従。
- 検証（2026-07-08・Chrome実データ）：物件番号15 土地933.32㎡・木造2階・価格応談 の取得OK。
